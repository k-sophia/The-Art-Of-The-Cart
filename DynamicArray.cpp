...
bool DynamicArray<T>::remove(const T &an_item)
{
    try
    {
        if (contains(an_item)) {
            ...
            int where_ = getIndexOf(an_item);
            T* newArr = new T[current_capacity_]; //new array
...
void DynamicArray<T>::resize()
{
    ...
    double reduce_req_ = current_capacity_ / 4;
    
    size_t new_size_ = current_capacity_;

    if (atCapacity()) {
        new_size_ *= 2;
    }
    else if (item_count_ <= reduce_req_) {
        new_size_ /= 2;
    }

    T* newArr = new T[new_size_]; //new array
    for(int i = 0; i < current_capacity_; i++) {
        newArr[i] = items_[i];
    }
